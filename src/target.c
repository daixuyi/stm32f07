...
void led_init(void)
{
	...
	led_inits.GPIO_PuPd = GPIO_PuPd_UP;
	led_inits.GPIO_Mode = GPIO_Mode_OUT;
	led_inits.GPIO_Speed = GPIO_Speed_50MHz;
	led_inits.GPIO_Pin = GPIO_Pin_0;

    GPIO_Init(GPIOA, &led_inits);
}
...
uart_send(const uint8_t c)
{
    ...
    UART_BASE->TDR = (uint16_t)c & 0x01FFu;
}
...
uart_try_receive(uint8_t *pc)
{
    ...
    {
        *pc = (uint16_t) (UART_BASE->RDR & (uint16_t) 0x01FF);
...
iflash_init(void)
{
    ...
    FLASH_Unlock();
    /* 清除状态 */
    FLASH_ClearFlag(FLASH_FLAG_EOP |FLASH_FLAG_BSY  | FLASH_FLAG_WRPERR |FLASH_FLAG_PGERR
             );
}
